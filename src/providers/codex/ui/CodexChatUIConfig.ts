...
import {
  codexModelSupportsFastServiceTier,
  getCodexDefaultModelIds,
...
const CODEX_SERVICE_TIER_TOGGLE: ProviderServiceTierToggleConfig = {
  ...
  activeValue: 'fast',
  activeLabel: 'Fast',
  description: 'Enable fast mode for this conversation. Faster responses use more credits.',
};
...
export const codexChatUIConfig: ProviderChatUIConfig = {
  ...
  isDefaultModel(model: string): boolean {
    return getCodexDefaultModelIds().has(model);
  },
  ...
  getCustomModelIds(envVars: Record<string, string>): Set<string> {
    ...
    if (envVars.OPENAI_MODEL && !getCodexDefaultModelIds().has(envVars.OPENAI_MODEL)) {
  ...
  getServiceTierToggle(settings): ProviderServiceTierToggleConfig | null {
    return codexModelSupportsFastServiceTier(settings.model as string | undefined)
