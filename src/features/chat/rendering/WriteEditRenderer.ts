...
export function updateWriteEditWithDiff(state: WriteEditState, diffData: ToolDiffData): void {
  ...
  const diffEl = row.createDiv({ cls: 'codexian-write-edit-diff' });
...
export function finalizeWriteEditBlock(state: WriteEditState, isError: boolean): void {
  ...
  state.statusEl.className = 'codexian-write-edit-status';
  ...
  if (isError) {
    ...
    if (!state.diffLines) {
      state.contentEl.empty();
      const row = state.contentEl.createDiv({ cls: 'codexian-write-edit-diff-row' });
      const errorEl = row.createDiv({ cls: 'codexian-write-edit-error' });
...
export function renderStoredWriteEdit(parentEl: HTMLElement, toolCall: ToolCallInfo): HTMLElement {
  ...
  const statusEl = headerEl.createDiv({ cls: 'codexian-write-edit-status' });
  if (isError) {
  ...
  const row = contentEl.createDiv({ cls: 'codexian-write-edit-diff-row' });
