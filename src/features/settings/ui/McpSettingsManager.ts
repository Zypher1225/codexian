...
export class McpSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'codexian-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'codexian-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'codexian-mcp-add-container' });
    ...
    const dropdown = addContainer.createDiv({ cls: 'codexian-mcp-add-dropdown' });

    const stdioOption = dropdown.createDiv({ cls: 'codexian-mcp-add-option' });
    setIcon(stdioOption.createSpan({ cls: 'codexian-mcp-add-option-icon' }), 'terminal');
    ...
    const httpOption = dropdown.createDiv({ cls: 'codexian-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'codexian-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'codexian-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'codexian-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'codexian-mcp-empty' });
      emptyEl.setText('No MCP servers configured. Click "Add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'codexian-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'codexian-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('codexian-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'codexian-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'codexian-mcp-status-enabled' : 'codexian-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'codexian-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'codexian-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'codexian-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'codexian-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'codexian-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'codexian-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'codexian-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'codexian-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'codexian-mcp-action-btn codexian-mcp-delete-btn',
