...
function inferSettingsProviderFromEnvironment(
  providerConfigs: ProviderConfigMap,
): string | null {
  const providerIds = Object.entries(providerConfigs)
    .filter(([, config]) => (
      typeof config?.environmentVariables === 'string'
      && config.environmentVariables.trim().length > 0
    ))
...
export class ClaudianSettingsStorage {
  ...
  async load(): Promise<StoredClaudianSettings> {
    ...
    if (!('settingsProvider' in stored)) {
      const inferredSettingsProvider = inferSettingsProviderFromEnvironment(
        merged.providerConfigs
      );
      if (inferredSettingsProvider) {
        merged.settingsProvider = inferredSettingsProvider;
      }
    }
