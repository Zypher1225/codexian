...
describe('FileContextManager', () => {
  ...
  it('renders current note chip and removes on click', () => {
    ...
    const indicator = findByClass(containerEl, 'codexian-file-indicator');
    expect(indicator).toBeDefined();
    expect(indicator?.style.display).toBe('flex');

    const removeEl = findByClass(containerEl, 'codexian-file-chip-remove');
  ...
  it('shows vault-relative path in @ dropdown and inserts full path on selection', () => {
    ...
    const pathEl = findByClass(containerEl, 'codexian-mention-path');
  ...
  it('wires getCachedVaultFolders through VaultFolderCache.getFolders', () => {
    ...
    const folderLabel = findByClass(containerEl, 'codexian-mention-name-folder');
  ...
  it('filters context files and attaches absolute path', () => {
    ...
    const nameEls = findAllByClass(containerEl, 'codexian-mention-name-context');
